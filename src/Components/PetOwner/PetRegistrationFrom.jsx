...
//     e.preventDefault();
//     try  {
//       const response = await axios.post("http://localhost:8080/api/pets/register-pet", formData, {
//         headers: {
...
const PetRegistrationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8080/api/pets/register-pet", payload, {
        headers: {
          "Content-Type": "application/json",
        }
      });
      ...
      if (response.status === 200 || response.status === 201) {
        alert("Pet registered successfully!");
        navigate("/profile", { state: { newPet: payload } });
      } else {
        alert("Failed to register pet.");
      }
    } catch (error) {
